...
struct usbtmc_message {
    uint32_t transfer_size;  /* size of bytes to transfer */
    uint32_t transferred;    /* size of received/written bytes */
    uint32_t flags;          /* bit 0: 0 = synchronous; 1 = asynchronous */
...
#define USBTMC_IOCTL_GET_TIMEOUT _IOR(USBTMC_IOC_NR, 9, uint32_t)
#define USBTMC_IOCTL_SET_TIMEOUT _IOW(USBTMC_IOC_NR, 10, uint32_t)
#define USBTMC_IOCTL_EOM_ENABLE _IOW(USBTMC_IOC_NR, 11, uint8_t)
#define USBTMC_IOCTL_CONFIG_TERMCHAR _IOW(USBTMC_IOC_NR, 12, struct usbtmc_termchar)
#define USBTMC_IOCTL_WRITE _IOWR(USBTMC_IOC_NR, 13, struct usbtmc_message)
#define USBTMC_IOCTL_READ _IOWR(USBTMC_IOC_NR, 14, struct usbtmc_message)
#define USBTMC_IOCTL_WRITE_RESULT _IOWR(USBTMC_IOC_NR, 15, uint32_t)
#define USBTMC_IOCTL_API_VERSION _IOR(USBTMC_IOC_NR, 16, uint32_t)
...
#define USBTMC488_IOCTL_WAIT_SRQ _IOW(USBTMC_IOC_NR, 23, uint32_t)

#define USBTMC_IOCTL_MSG_IN_ATTR _IOR(USBTMC_IOC_NR, 24, uint8_t)
#define USBTMC_IOCTL_AUTO_ABORT _IOW(USBTMC_IOC_NR, 25, uint8_t)

#define USBTMC_IOCTL_GET_STB _IOR(USBTMC_IOC_NR, 26, uint8_t)
#define USBTMC_IOCTL_GET_SRQ_STB _IOR(USBTMC_IOC_NR, 27, uint8_t)
